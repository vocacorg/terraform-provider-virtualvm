...
// PipelinesEnabled is the struct we send to turn on or turn off pipelines for a Sample
type PipelinesEnabled struct {
	Enabled bool `json:"enabled"`
}

// Sample is the struct we need to send off to the Bitbucket API to create a Sample
type Sample struct {
...
func resourceSample() *schema.Resource {
	return &schema.Resource{
		Create: resourceSampleCreate,
		Update: resourceSampleUpdate,
		Read:   resourceSampleRead,
		Delete: resourceSampleDelete,
...
func newSampleFromResource(d *schema.ResourceData) *Sample {
	repo := &Sample{
...
func resourceSampleUpdate(d *schema.ResourceData, m interface{}) error {
	client := m.(*Client)
	Sample := newSampleFromResource(d)
	...
	return resourceSampleRead(d, m)
}

func resourceSampleCreate(d *schema.ResourceData, m interface{}) error {
	client := m.(*Client)
	repo := newSampleFromResource(d)
...
func resourceSampleRead(d *schema.ResourceData, m interface{}) error {
	id := d.Id()
	if id != "" {
...
func resourceSampleDelete(d *schema.ResourceData, m interface{}) error {
